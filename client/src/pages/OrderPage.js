...
export default function OrderPage(props) {
  ...
  useEffect(() => {
    ...
    const addPayPalScript = async () => {
       /* here data contains the clientID*/
    ...
    if (!order || successPay || successDeliver ||(order && order._id !== orderId)) {
      ...
      if (!order.isPaid) {
          // checking if haven't already load the paypal
        if (!window.paypal) {
          addPayPalScript();
        } else {
          setSdkReady(true);
...
// payment result is the result of your payment on paypal
  ...
  return loading ? (
    ...
    <MessageBox variant="danger">{error}</MessageBox>
      ...
      // if no loading and no error, go for data
    <div>
        ...
        <div className="col-1">
          <div className="card card-body">
            <ul>
              ...
              {!order.isPaid && (
                <li>
                    {/* check if sdkready is false mean that  still loading the people
                    and show loading box otherwise show pappal button */}
                  {!sdkReady ? (
                    <LoadingBox></LoadingBox>
                  ) : (
                    <>
